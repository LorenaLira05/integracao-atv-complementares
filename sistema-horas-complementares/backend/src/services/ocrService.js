...
// Função para extrair texto direto de PDFs digitais (pesquisáveis)
const extrairTextoPdfDireto = async (caminhoArquivo) => {
    try {
        ...
        console.warn("[PDF Direct] Falha ao extrair texto direto do PDF:", err.message);
        return "";
    }
};
...
exports.executarOCR = async (caminhoArquivo, mimetype) => {
    try {
        ...
        if (ehPdf) {
            // 1. Tentar extração direta do texto digital do PDF
            const textoDireto = await extrairTextoPdfDireto(caminhoArquivo);
            
            // Se o texto direto for longo o suficiente (ex: > 30 caracteres), usamos
            if (textoDireto && textoDireto.length > 30) {
                console.log(`[OCR] Texto extraído diretamente do PDF (${textoDireto.length} caracteres).`);
                resultadoOCR = {
                    texto: textoDireto,
                    confianca: 100
                };
            } else {
                console.log("[OCR] PDF digital não contém texto extraível ou é imagem. Iniciando conversão de página e OCR Tesseract...");
        ...
        // Passa o texto extraído para a Groq estruturar
